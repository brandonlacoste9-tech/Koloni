  ...
  try {
    ...
    if (!prompt || !userId) {
      return {
        statusCode: 400,
        body: JSON.stringify({
          error: "Missing required parameters: prompt, userId",
        }),
      };
    }
          ...
          messages: [
            {
              role: "system",
              content:
                "You are a creative AI assistant specializing in generating engaging Emu format content. Emu content is concise, impactful, and optimized for quick consumption.",
            },
            {
              role: "user",
    ...
    await fetch(`${process.env.URL}/.netlify/functions/token-manager`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
